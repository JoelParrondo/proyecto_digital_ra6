# 🚀 Proyecto de Transformación Digital (RA6)

## 🧑‍💼 1. Empresa
- **Nombre de la empresa:** TechMobile Solutions  
- **Sector:** Tecnología / Comercio electrónico  
- **Descripción breve:** Empresa dedicada a la venta online de smartphones, accesorios y servicios técnicos.

---

## 🎯 2. Objetivos estratégicos (SMART)

### Objetivo 1
- **Descripción:** Incrementar las ventas online mediante optimización web  
- **Métrica:** +25% en ventas mensuales  
- **Plazo:** 6 meses  

### Objetivo 2
- **Descripción:** Mejorar la experiencia del cliente en la web  
- **Métrica:** Reducir la tasa de rebote en un 15%  
- **Plazo:** 4 meses  

### Objetivo 3
- **Descripción:** Automatizar la gestión de pedidos  
- **Métrica:** Reducir el tiempo de procesamiento en un 30%  
- **Plazo:** 5 meses  

---

## 🏢 3. Áreas de la empresa

| Área | Función | Sub-objetivo digital | Impacto esperado |
|------|--------|----------------------|------------------|
| Marketing | Promoción | SEO y campañas digitales | Mayor visibilidad |
| Ventas | Comercialización | Implementación eCommerce | Aumento de ingresos |
| Logística | Envíos | Automatización de pedidos | Reducción de errores |
| Atención al cliente | Soporte | Chatbots | Mejora de satisfacción |

---

## ⚙️ 4. Tecnologías habilitadoras digitales (THD)

- **Inteligencia Artificial**
  - Uso en la empresa: Chatbots para atención al cliente  

- **Big Data**
  - Uso en la empresa: Análisis de comportamiento de usuarios  

- **Cloud Computing**
  - Uso en la empresa: Almacenamiento en la nube  

- **CMS (WordPress + Blocksy)**
  - Uso en la empresa: Gestión del sitio web  

- **Automatización (RPA)**
  - Uso en la empresa: Gestión automática de pedidos  

---

## 📊 5. Datos y tratamiento de la información

- **Tipos de datos que genera la empresa:**  
  Datos de clientes, pedidos, navegación web  

- **Cómo se capturan:**  
  Formularios web, cookies, compras online  

- **Cómo se almacenan:**  
  Bases de datos en la nube  

- **Cómo se procesan:**  
  Sistemas ERP y CRM  

- **Cómo se analizan:**  
  Herramientas de analítica web  

- **Uso en la toma de decisiones:**  
  Optimización de campañas y productos  

---

## 🔐 6. Seguridad y riesgos

### Riesgos identificados
1. Ciberataques  
2. Robo de datos  
3. Fallos del sistema  

### Medidas de protección
1. Certificados SSL y cifrado  
2. Copias de seguridad  
3. Firewalls y antivirus  

---

## 👨‍💻 7. Recursos humanos

- **Nuevos perfiles necesarios:**  
  Especialista en marketing digital, analista de datos  

- **Formación requerida:**  
  SEO, analítica web, ciberseguridad  

- **Cambios organizativos:**  
  Digitalización de procesos y trabajo remoto  

---

## 🔗 8. Integración de sistemas

- **ERP utilizado o propuesto:** Odoo  
- **CRM utilizado o propuesto:** HubSpot  
- **BPM / automatización:** Zapier  
- **Integración entre sistemas:** API y sincronización en la nube  

---

## 💡 9. Valor del proyecto

- **Beneficios económicos:** Incremento de ventas  
- **Mejora operativa:** Automatización y eficiencia  
- **Ventaja competitiva:** Mejor experiencia de usuario  
- **Riesgos de no digitalizar:** Pérdida de clientes y competitividad  

---

## 📐 10. Modelado UML (OBLIGATORIO)

### Casos de uso
- **Actores:** Cliente, Administrador  
- **Funcionalidades principales:**  
  - Comprar producto  
  - Gestionar pedidos  
  - Atender consultas  

### Diagrama de clases
- **Entidades principales:**  
  Cliente, Pedido, Producto, Pago  

- **Relaciones:**  
  - Cliente realiza Pedido  
  - Pedido contiene Productos  

### Diagrama de secuencia (opcional)
- **Caso representado:**  
  Proceso de compra online  

---
